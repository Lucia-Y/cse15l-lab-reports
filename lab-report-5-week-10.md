...
The problem for the code is that it still considers it as a link when there is ``!`` in front of the ``[]``. In order to fix the code, we need to add another condition that when there is ``!`` in front of the ``[]``, the while loop should ``break``.
...
The problem for the code is the same as the problem for test 571, which is that it still considers it as a link when there is ``!`` in front of the ``[]``. In order to fix the code, we need to add another condition that when there is ``!`` in front of the ``[]``, the while loop should ``break``.
